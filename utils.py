...
def probability(percentage: float) -> bool:
...
class Position:
    ...
    def multiplier(self, x: float, y: float):
        self.x *= x
        self.y *= y
        return self  # returning self to allow chaining

    def multiplier_new(self, x: float, y: float):
        return Position.from_tuple((self.x * x, self.y * y))
